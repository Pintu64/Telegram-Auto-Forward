...
Get `API_ID` and `API_HASH` from https://my.telegram.org and a bot token from `@BotFather`. The first login asks for your phone number, Telegram code, and 2FA password if configured. The resulting session file is a login credential. Do not share it.
