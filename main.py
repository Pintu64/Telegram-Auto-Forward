...
async def run():
    ...
    Path(config.session_name).parent.mkdir(parents=True, exist_ok=True); database = Database(config.database_path); client = TelegramClient(config.session_name, config.api_id, config.api_hash); await client.connect()
    if not await client.is_user_authorized(): await client.disconnect(); database.close(); raise RuntimeError("User session is not authorized. Run: python login.py")
