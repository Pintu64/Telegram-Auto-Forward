...
async def run():
    ...
    service = ForwardingService(client, database); service.register(); await service.sync_recent_history(config.history_limit); control = ControlBot(config.bot_token, config.owner_user_id, database, service)
