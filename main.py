...
async def run():
    ...
    service = ForwardingService(client, database, config.history_limit); service.register(); await service.sync_recent_history(); control = ControlBot(config.bot_token, config.owner_user_id, database, service)
